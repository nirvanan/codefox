...
{
	gtk_widget_destroy (new_project_dialog->toplevel);
...
{
	gtk_widget_destroy (create_file_dialog->toplevel);
...
{
	gtk_widget_destroy (project_settings_dialog->toplevel);
...
ui_filetree_menu_init ()
{
	...
	filetree_menu->menu = gtk_menu_new ();
	filetree_menu->add_item = gtk_image_menu_item_new_with_label (_("Add local file"));
	filetree_menu->create_item = gtk_image_menu_item_new_with_label (_("Create empty file"));
	filetree_menu->delete_item = gtk_image_menu_item_new_with_label (_("Delete this item"));
	...
	gtk_image_menu_item_set_image (GTK_MENU_ITEM (filetree_menu->create_item), stock_new);
	gtk_image_menu_item_set_image (GTK_MENU_ITEM (filetree_menu->add_item), stock_open);
	gtk_image_menu_item_set_image (GTK_MENU_ITEM (filetree_menu->delete_item), stock_delete);
	gtk_menu_shell_append (GTK_MENU_SHELL (filetree_menu->menu), GTK_MENU_ITEM (filetree_menu->create_item));
	gtk_menu_shell_append (GTK_MENU_SHELL (filetree_menu->menu), GTK_MENU_ITEM (filetree_menu->add_item));
	gtk_menu_shell_append (GTK_MENU_SHELL (filetree_menu->menu), GTK_MENU_ITEM (filetree_menu->delete_item));
...
ui_error_dialog_new (const gchar *message)
{
	...
	dialog = gtk_message_dialog_new (window->toplevel, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
...
ui_confirm_dialog_new (const gchar *message)
{
	...
	dialog = gtk_message_dialog_new (window->toplevel, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
...
ui_compiletree_apend (const gchar *line, gint is_message)
{
	...
	gtk_notebook_set_current_page (GTK_NOTEBOOK (window->info_notebook), 
								   1);
}
...
ui_function_autocomplete (const gchar *name, const GList *signs)
{
	...
	if (function_tip->active)
		gtk_widget_destroy (function_tip->tip_window);

	function_tip->tip_window = ui_tip_window_new (full);
	ui_current_editor_insert_location (&x, &y);
	gtk_window_move (GTK_WINDOW (function_tip->tip_window), x, y);
	gtk_widget_show_all (function_tip->tip_window);
...
ui_member_autocomplete (const GList *funs, const GList *vars)
{
	...
	for (iterator = funs; iterator; iterator = iterator->next) {
		...
		gtk_menu_shell_append (GTK_MENU_SHELL (member_menu->menu), GTK_MENU_ITEM (item));
		g_signal_connect (item, "activate", 
					  	  G_CALLBACK (on_autocomplete_item_clicked), NULL);

		gtk_widget_show (item);
		member_menu->item_list = g_slist_append (member_menu->item_list, item);
	}
	...
	gtk_widget_override_font (member_menu->menu, pfd);
...
ui_current_editor_insert_location(gint *x, gint *y)
{
	...
	/* FIXME */
	*x += gtk_paned_get_position (window->horizontal_paned);
}
...
ui_member_menu_destroy ()
{
	...
	gtk_widget_destroy (member_menu->menu);
	g_slist_free (member_menu->item_list);
	member_menu->item_list = NULL;
	member_menu->prefix[0] = 0;
	member_menu->active = FALSE;
}
...
ui_member_menu_item_filter ()
{
	...
	for (iterator = member_menu->item_list; iterator; iterator = iterator->next) {
		...
		if (g_str_has_prefix (label, member_menu->prefix) && g_strcmp0 (label, member_menu->prefix) != 0) {
			gtk_widget_show (item);
			item_showed++;
		}
		else
...
ui_member_menu_update (const gboolean del, const gchar ch)
{
	...
	if (item_showed == 0)
		gtk_widget_hide (member_menu->menu);
	else {
...
ui_status_image_set (const gboolean error, const gboolean warning)
{
	...
	if (error)
		gtk_image_set_from_stock (window->status_image, CODEFOX_STOCK_STATUSERROR, GTK_ICON_SIZE_MENU);
	else if (warning)
		gtk_image_set_from_stock (window->status_image, CODEFOX_STOCK_STATUSWARNING, GTK_ICON_SIZE_MENU);
	else
		gtk_image_set_from_stock (window->status_image, CODEFOX_STOCK_STATUSPASS, GTK_ICON_SIZE_MENU);
...
{
	gtk_widget_show_all (preferences_window->toplevel);
}

void
ui_preferences_window_hide ()
{
	gtk_widget_hide (preferences_window->toplevel);
}
...
ui_debug_ptr_add (const gchar *filepath, const gint line)
{
	...
	if (filepath == NULL) {
		editor = ui_get_current_editor ();
		window->debug_ptr = ceditor_icon_add (editor, line);
...
{
	debug_view_calltree_add (window->debug_view, frame_name, frame_args,
							 file_line);
}
