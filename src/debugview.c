...
debugview_disable (CDebugView *debug_view)
{
	...
	store = GTK_LIST_STORE(gtk_tree_view_get_model(debug_view->localtree));
	gtk_list_store_clear (store);
	store = GTK_LIST_STORE(gtk_tree_view_get_model(debug_view->calltree));
	gtk_list_store_clear (store);
	store = GTK_LIST_STORE(gtk_tree_view_get_model(debug_view->watchtree));
	gtk_list_store_clear (store);
	gtk_list_store_append (store, &iter);
	gtk_list_store_set(store, &iter, 0, "", 1, "", -1);
...
debugview_watchtree_get_all (CDebugView *debug_view, GList **list)
{
	...
	*list = g_slist_append (*list, (gpointer) line);
